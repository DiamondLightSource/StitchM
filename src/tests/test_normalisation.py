...
class NormalisationTests(unittest.TestCase):
    ...
    def test_normalise_to_datatype(self):
        ...
        self.assertTrue(
            (image_out.max() == self.image_max),
            msg=f"image max {image_out.max()} is not {self.image_max}",
        )
        self.assertTrue(
            (image_out.min() == 0), msg=f"image min {image_out.min()} is not 0"
        )
    ...
    def test_normalise_simple_minmax_image(self):
        ...
        expected_image = np.around(
            np.linspace(0, self.image_max, 12).reshape(3, 4), 0
        ).astype(self.dtype)
