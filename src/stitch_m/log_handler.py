...
class LogHandler:
    def __init__(
        ...
        self.logger = self._create_or_setup(
            file_level, stream_level, log_to_file, backup_count, config, config_messages
        )
    ...
    def _create_or_setup(
        ...
        if config:
            ...
            logger = self.create_logger(
                file_level, stream_level, log_to_file, backup_count
            )
    ...
    def setup_logging(self, config):
        ...
        try:
            ...
            logging.error(
                "Error retrieving logging levels from config file. Using verbose settings."
            )
            return self.create_logger()

    @staticmethod
    def create_logger(
        file_level="debug", stream_level="info", log_to_file=False, backup_count=10
    ):
        """
        ...
        if stream_level in level_dict:
            ...
            logging.error(
                "Invalid logging level passed. Stream logging will be set to ERROR"
            )
