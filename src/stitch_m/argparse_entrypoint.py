...
def main():
    ...
    stitch_group.add_argument(
        "-a",
        "--markers",
        metavar="PATH/TO/MARKER_FILE.TXT",
        dest="markers",
        type=str,
        default="",
        action="store",
    ...
    setup_subparsers = parser.add_subparsers(
        title="Setup options", description="enter `StitchM setup -h` for details"
    )
    setup_parser = setup_subparsers.add_parser(name="setup", add_help=False)
    setup_parser.add_argument(
        "-w",
        "--windows-shortcut",
        dest="windows_shortcut",
        action="store_true",
        help=(
    ...
    if args.mosaic:
        ...
        try:
            ...
            logging.error(
                "Error has occurred. Please see traceback for more info.", exc_info=True
            )
    else:
        parser.print_help()
