...
class UnstitchedImage:
    def __init__(self, filepath):
        ...
        # Open mrc file:
        _logger.info("Opening mrc file at %s", str(mrc_path))
        ...
        self.pixel_size = (
            self.img_size[0] / self.img_pixels[0]
    ...
    def __calculate_boundaries(self):
        min_coords = (
            self.pix_positionlist[:, 0].min(),
            self.pix_positionlist[:, 1].min(),
        )
        max_coords = (
            self.pix_positionlist[:, 0].max(),
            self.pix_positionlist[:, 1].max(),
        )

        return np.array(
            [
                [min_coords[0] - self.pix2edge[0], min_coords[1] - self.pix2edge[1]],
                [max_coords[0] + self.pix2edge[0], max_coords[1] + self.pix2edge[1]],
            ],
            dtype=int,
        )
